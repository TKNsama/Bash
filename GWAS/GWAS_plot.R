...
plot_gwas_results <- function(file_prefix, threshold_logp = 2, af_threshold = 0.1) {
  ...
  # filter
  df_filtered <- df[df$log_p_value > threshold_logp & df$af > af_threshold, ]
  write_tsv(df_filtered, paste0(file_prefix, "_filter.tsv"))
  
  # 绘制 QQ plot
  df_qq <- df_filtered[,c(2, 1, 3, 12)]
...
plot_gwas_results <- function(file_prefix, threshold_logp = 2) {
  ...
  if (!file.exists(filter_file)) {
    message(paste0("Skipping ", file_prefix, " due to missing filtered file."))
    return(NULL)
  }
  
  df_filtered <- read_tsv(filter_file)
  
  # 生成 QQ plot
  df_qq <- df_filtered[, c(2, 1, 3, 12)]
  CMplot(df_qq, plot.type = "q",
         file.output = TRUE,
  ...
  # 生成 Manhattan plot
  df_manhattan <- df_filtered[, c(2, 1, 3, 12)]
  if (nrow(df_manhattan) > 0) {
    CMplot(df_manhattan, plot.type = "m",
           threshold = c(0.01, 0.05) / nrow(df_manhattan),
           threshold.col = c('grey', 'black'),
           threshold.lty = c(1, 2),
